  ...
  <div class="prodetail">
    ...
    <div class="comment">
      <div class="comment-title">
        <div class="left">商品评价 ({{ comments_const }}条)</div>
    ...
    <van-action-sheet v-model="showPannel" :title="mode === 'cart' ? '加入购物车' : '立刻购买'">
      <div class="product">
        ...
        <div class="showbtn" v-if="detail.stock_total > 0">
          ...
          <div class="btn now" @click="buyNow" v-else>立刻购买</div>
        </div>
...
<script>
import { getProDetail, getProComments } from '@/api/products'
...
export default {
  ...
  async created () {
    this.detail = (await getProDetail(this.goodsId)).data.detail
    this.images = this.detail.goods_images
    const { data: { list, total } } = await getProComments(this.goodsId, 3)
    this.comments = list
    this.comments_const = total
  }
}
</script>
