    ...
    <div class="cart-box" v-if="isLogin && cartList.length > 0">
      ...
      <div class="footer-fixed">
        ...
        <div class="all-total">
          ...
          <div v-if="!isEdit" @click="goPay" :class="{ disabled: selCount === 0 }" class="goPay">结算({{ selCount }})</div>
          <div @click="delSelect" v-else :class="{ disabled: selCount === 0 }" class="delete">删除</div>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    changeCount (value, goodsId, skuId) {
      console.log(value, goodsId, skuId)
    ...
    goPay () {
      if (this.isLogin && this.selCartList) {
        this.$router.push({
          path: '/pay',
          query: {
            mode: 'cart',
